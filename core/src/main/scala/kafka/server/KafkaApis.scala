 ...
 * @param adminManager     主题/分区/配置管理器
 * @param groupCoordinator 消费者协调组件
...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  private def createTopic(topic: String,
    ...
    try {
      ...
      // Broker 端创建完主题后，会显式地通知 Clients 端 LEADER_NOT_AVAILABLE 异常。
      // Clients 端接收到该异常后，会主动更新元数据，去获取新创建主题的信息。
  ...
  // 授权检查
  // 所有请求在处理前，都需要调用 authorize 方法做权限校验，以保证请求能够被继续执行。
   ...
   * Processor 线程取出该类型的 Response 后，不执行真正的 I/O 发送操作。
   */
